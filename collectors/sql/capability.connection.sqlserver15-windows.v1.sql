...
SELECT TOP (1)
    ...
    CONVERT(bit, COALESCE(HAS_PERMS_BY_NAME(NULL, NULL, N'VIEW SERVER STATE'), 0)) AS has_required_permission,
    CAST(0 AS bit) AS is_performance_reader_member,
    ...
    CONVERT(nvarchar(40), connection_info.encrypt_option) AS encrypt_option
FROM sys.dm_os_host_info AS host
CROSS JOIN
(
    SELECT TOP (1) current_connection.encrypt_option
    FROM sys.dm_exec_connections AS current_connection
    WHERE current_connection.session_id = @@SPID
) AS connection_info
WHERE CONVERT(int, SERVERPROPERTY(N'ProductMajorVersion')) = 15;
