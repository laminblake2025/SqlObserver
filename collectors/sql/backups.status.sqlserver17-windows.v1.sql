...
(
 ...
 AND (b.type <> 'D' OR b.is_copy_only = 0)
)
SELECT TOP (@maximum_rows) CONVERT(binary(32), HASHBYTES('SHA2_256', database_name)) AS database_fingerprint,
 ...
 has_backup_checksums AS has_checksum, is_damaged AS is_damaged, TRY_CONVERT(bigint,backup_set_id) AS backup_set_id,
 CONVERT(smallint,CASE WHEN time_zone BETWEEN -48 AND 48 THEN time_zone * 15 ELSE NULL END) AS time_zone_offset_minutes
