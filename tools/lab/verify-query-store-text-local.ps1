...
function Invoke-ProbeSql([string] $databaseName, [string] $statement) {
    $output = & sqlcmd -S $SqlInstance -d $databaseName -E -b -W -h -1 -s '|' -l 5 -t 20 -Q $statement 2>&1
...
try {
    ...
    if ($fields.Count -ne 17) { throw 'Pinned Query Store metadata changed its bounded row shape.' }
    ...
    Write-Output "Pinned SQL Server 16 metadata and bounded text lookup returned synthetic workload text ID $textId."
}
finally {
    if ($created) {
