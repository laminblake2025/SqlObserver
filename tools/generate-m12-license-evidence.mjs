...
export async function buildLicenseEvidence({ sbomPath, sbom, nugetRoot, npmRoot, licenseMap, licenseMapPath, commitSha, runId, sbomRunId = runId, environmentId }) {
  if (!COMMIT.test(commitSha ?? "")) fail("commitSha is invalid"); if (!ENVIRONMENTS.has(environmentId)) fail("environmentId is invalid"); if (typeof runId !== "string" || !RUN_ID.test(runId) || typeof sbomRunId !== "string" || !RUN_ID.test(sbomRunId)) fail("runId is invalid");
  ...
  const thirdParty = validateSbom(sbomInput.value, { commitSha, runId: sbomRunId, environmentId }); if (!sbom && !Buffer.from(jsonBytes(sbomInput.value)).equals(sbomInput.bytes)) fail("SBOM bytes are not canonical"); let map; if (licenseMap !== undefined) map = licenseMap; else if (licenseMapPath) { const mapFile = path.resolve(licenseMapPath); await assertTrustedPath(path.dirname(mapFile), mapFile); map = (await readClosed(mapFile, "license map")).value; } if (map !== undefined) object(map, "license map");
