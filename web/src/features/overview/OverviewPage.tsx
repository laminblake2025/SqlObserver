...
export function OverviewPage({refresh,onAdd,canAddServer}: {refresh:number;onAdd:()=>void;canAddServer:boolean}) {
  ...
  return <div className="overview-page">
    ...
    {data&&<><div className="overview-coverage" role="status"><span className={current===data.evidence.length?'coverage-dot':'coverage-dot partial'}/><strong>{current}/{data.evidence.length} servers reporting current SQL core evidence</strong><span>{data.excludedTargets} disabled/retired excluded from All servers</span><small>Refreshed {new Date(data.refreshedAtUtc).toISOString().replace('T',' ')} · SQL core freshness does not imply coverage for other sources</small></div>
      ...
      <section className="panel overview-panel overview-attention"><p className="eyebrow">INVESTIGATE FIRST</p><h2>Where to focus</h2>{issues.length?issues.map((issue,i)=><a className="overview-issue" key={`${issue.targetId}:${i}`} href={issueHref(scope,issue.targetId,issue.destination,issue.observedAtUtc)}><span className="issue-order">{i+1}</span><div><strong>{issue.title}</strong><span>{issue.server}</span><small>{issue.detail}</small><small>{issue.observedAtUtc??'Observation time unavailable'} · Open investigation →</small></div></a>):<p className="overview-empty">No actionable exceptions in the available evidence. Check collection coverage before concluding the environment is healthy.</p>}</section></div>
      <section className="panel overview-panel overview-servers"><div className="overview-panel-heading"><div><p className="eyebrow">AUTHORIZED TARGETS</p><h2>Servers</h2></div><label className="inline-search">Search loaded servers<input aria-label="Search loaded servers" placeholder="Search loaded targets…" value={serverSearch} onChange={e=>setServerSearch(e.target.value)}/></label></div><p className="table-note">Showing {data.targets.filter(target=>`${target.displayName} ${target.targetId}`.toLowerCase().includes(serverSearch.toLowerCase())).length} of {data.targets.length} targets from this overview response. Search does not load additional targets.</p><div className="table-scroll"><table><caption>SQL core freshness and latest target-scoped counts</caption><thead><tr><th scope="col">Server</th><th scope="col">SQL core evidence</th><th scope="col">Active alerts</th><th scope="col">Blocked sessions</th><th scope="col">Last observed</th></tr></thead><tbody>{data.targets.filter(target=>`${target.displayName} ${target.targetId}`.toLowerCase().includes(serverSearch.toLowerCase())).map(target=>{const item=data.evidence.find(e=>e.targetId===target.targetId);return <tr key={target.targetId}><td><a href={href('health',target.targetId)}>{target.displayName}</a><small>{target.targetId}</small></td><td><span className={`state-pill state-${item?.collectionState??'unavailable'}`}>{formatCollectionState(item?.collectionState)}</span></td><td>{formatOverviewCount(item?.activeAlerts)}</td><td>{formatOverviewCount(item?.blockedSessions)}</td><td>{item?.lastObservedUtc??'Unavailable'}</td></tr>;})}</tbody></table></div>{data.targets.length===0?<p className="empty-state">No authorized targets are available in this response.</p>:null}</section>
      ...
      <details className="panel overview-panel"><summary>Collection coverage and evidence gaps</summary>{data.evidence.map(e=><div className="overview-gap" key={e.targetId}><a href={href('health',e.targetId)}>{e.displayName}</a><span>SQL collection: {e.collectionState} · Last success: {e.lastObservedUtc??'unavailable'}</span>{e.gaps.map((gap,i)=><small key={i}>{gap}</small>)}</div>)}<p className="muted">{overviewEvidenceFooter(data)}</p></details>
...
function formatCollectionState(value: string | undefined): string {
  if (value === undefined) return "Unavailable";
  return value.replaceAll("_", " ").replace(/\b\w/g, (character) => character.toUpperCase());
}

function formatOverviewCount(value: { readonly value: number | null; readonly state: string } | undefined): string {
