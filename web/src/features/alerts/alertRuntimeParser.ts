...
export function parseActiveAlerts(value: unknown, targetId: string): ActiveAlertPage {
  ...
  const items = row.items.map((item): ActiveAlert => {
    ...
    if (candidate.ruleName !== undefined && candidate.ruleName !== null && (typeof candidate.ruleName !== "string" || candidate.ruleName.length > 200)) throw new Error("Alerts response is invalid.");
    return { ruleName: typeof candidate.ruleName === "string" ? candidate.ruleName : undefined, alertId: candidate.alertId, ruleId: candidate.ruleId, state: candidate.state, firstObservedUtc: candidate.firstObservedUtc, firedUtc: optionalUtc(candidate.firedUtc), acknowledgedUtc: optionalUtc(candidate.acknowledgedUtc), value: candidate.value as number | null | undefined, reason: candidate.reason as string | undefined, deliverySuppressed: candidate.deliverySuppressed };
  });
