...
export async function getActiveAlerts(instanceId: string, signal: AbortSignal, limit = 100, cursor?: string): Promise<ActiveAlertPage> {
  ...
  if (!response.ok) throw responseFailure(response, response.status === 403 ? "You are not authorized to view alerts for this target." : "Alerts are temporarily unavailable.");
...
export async function acknowledgeAlert(instanceId: string, alertId: string, signal: AbortSignal): Promise<void> {
  ...
  if (!response.ok) throw responseFailure(response, response.status === 403 ? "You are not authorized to acknowledge alerts." : "The alert could not be acknowledged.");
}
