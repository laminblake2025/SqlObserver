...
export function startLiveRefresh(run: () => Promise<void>, onFailure: (error: unknown) => void, signal: AbortSignal, repeat: boolean,
  ...
  async function tick() {
    ...
    catch (error) { if (!signal.aborted) { failures++; onFailure(error); } }
    finally {
