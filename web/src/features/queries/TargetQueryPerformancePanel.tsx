...
import { getQueryPerformanceHistory, getQueryPerformancePlan, getQueryPerformanceStatus, getQueryPerformanceText, getQueryPerformanceTop } from "./queryPerformanceApi";
...
export function TargetQueryPerformancePanel({ instanceId, displayName, onClose, timeWindow, onSelectWindow, refresh, canReadText }: {
  ...
  useEffect(() => () => { selectionRequest.current?.abort(); historyRequest.current?.abort(); pagingRequest.current?.abort(); textRequest.current?.abort(); }, [instanceId, ranking, timeWindow.fromUtc, timeWindow.toUtc]);
  ...
  useEffect(() => { if (!canReadText) { textRequest.current?.abort(); setTextResult(undefined); } }, [canReadText]);
  const clearText = () => { textRequest.current?.abort(); setTextResult(undefined); };
  ...
  useEffect(() => {
    ...
    const c = new AbortController(); clearText(); setPaging(false); setPage(undefined); setStatus(undefined); setMessage(undefined);
  ...
  const select = (databaseId: number, queryFingerprint: string, from: string, to: string, planFingerprint: string | undefined) => {
    ...
    clearText(); setPlan(undefined); setMessage(undefined); setHistorySelection(selection); setHistory(undefined);
  ...
  const loadMetricPage = () => {
    ...
    void getQueryPerformanceTop(instanceId, ranking, page.nextCursor, page.fromUtc, page.toUtc, c.signal).then(next => {
      ...
      clearText(); setHistorySelection(undefined); setHistory(undefined); setPlan(undefined); setPage(next);
  ...
  return <section className="query-performance-screen" aria-labelledby="query-performance-heading">
    ...
    {page === undefined ? null : <>
      ...
      <div className="query-investigation"><section className="panel"><h2>Query CPU and duration history</h2>
        ...
        <p>Plan content unavailable</p></section></div>
