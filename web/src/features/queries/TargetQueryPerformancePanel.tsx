import { metricsForObservation } from "./querySelection";
...
export function TargetQueryPerformancePanel({ instanceId, displayName, onClose }: { readonly instanceId: string; readonly displayName: string; readonly onClose: () => void }) {
  ...
  const [source, setSource] = useState("query_store");
  useEffect(() => () => { selectionRequest.current?.abort(); historyRequest.current?.abort(); pagingRequest.current?.abort(); }, [instanceId, ranking]);
  ...
  useEffect(() => {
    ...
    void getQueryPerformanceTop(instanceId, ranking, undefined, undefined, undefined, c.signal).then(value => { if (!c.signal.aborted) setPage(value); }).catch((e: unknown) => { if (!c.signal.aborted) setMessage(e instanceof Error ? e.message : "Query performance evidence is unavailable."); });
    void getQueryPerformanceStatus(instanceId, undefined, undefined, c.signal).then(value => { if (!c.signal.aborted) setStatus(value); }).catch((e: unknown) => { if (!c.signal.aborted) setMessage(e instanceof Error ? e.message : "Query performance status is unavailable."); });
    return () => c.abort();
  }, [instanceId, ranking]);
  ...
  return <section className="activity-panel" aria-labelledby="query-performance-heading">
    ...
    {page === undefined ? null : <>
      ...
      <p className="activity-evidence">Ranking window: {page.fromUtc} to {page.toUtc} · UTC · each row is one observation. Only the selected ranking metric is loaded; other metrics remain unavailable.</p>
      <label>Database <select value={database} onChange={event => { selectionRequest.current?.abort(); historyRequest.current?.abort(); setDatabase(event.target.value); setPlan(undefined); setHistory(undefined); setHistorySelection(undefined); }}><option value="">All loaded databases</option>{[...new Set(page.items.map(item => item.query.databaseId))].sort((a,b)=>a-b).map(id => <option key={id} value={id}>Database {id}</option>)}</select></label>
      <label>Source <select value={source} onChange={event => { selectionRequest.current?.abort(); historyRequest.current?.abort(); setSource(event.target.value); setPlan(undefined); setHistory(undefined); setHistorySelection(undefined); }}><option value="query_store">Query Store · interval</option><option value="plan_cache">Plan cache · row semantics</option><option value="mixed">Mixed source evidence</option></select></label>
      ...
      <div className="query-investigation"><section className="panel"><h2>Query duration history</h2>{history ? (source === "mixed" ? <p>Select Query Store or Plan cache to chart one source with consistent semantics.</p> : <ObservationChart items={history.items.filter(x => x.source === source && x.semantics === selectedItem?.semantics).map(x => ({time: x.intervalEndUtc, value: x.metrics.durationMilliseconds ?? null}))} label={`Duration (ms) · ${selectedItem?.semantics ?? "semantics unavailable"}`}/>) : <p>Select a query to load its bounded history.</p>}</section><section className="panel"><h2>Selected query</h2><p className="fingerprint">{historySelection?.queryFingerprint ?? "No query selected"}</p><p>Database {historySelection?.databaseId ?? "—"} · {source.replaceAll("_", " ")}</p><p>{selectedItem?.semantics ?? "Semantics unavailable"} · {selectedItem?.coverage ?? "Coverage unavailable"}</p><p className="fingerprint">Plan: {plan?.planFingerprint ?? "Metadata unavailable"}</p><p>Query text unavailable</p><p>Plan content unavailable</p></section></div>
      <div className="table-scroll"><table><caption>Bounded ranked observations · CPU and duration in milliseconds · unavailable values are not zero</caption><thead><tr>{["Database / query", "Source", "Semantics", "CPU ms", "Duration ms", "Executions", "Reads / writes / rows", "Coverage"].map(label => <th scope="col" key={label}>{label}</th>)}</tr></thead><tbody>
        {rows.map(x => <tr key={`${x.collectionRunId}-${x.observationKey}`} className={historySelection?.queryFingerprint === x.query.queryFingerprint && historySelection.databaseId === x.query.databaseId ? "selected-row" : ""}><td><button className="fingerprint" type="button" onClick={() => select(x.query.databaseId, x.query.queryFingerprint, x.intervalStartUtc, x.intervalEndUtc, x.plan?.planFingerprint)}>{x.query.databaseId} / {x.query.queryFingerprint.slice(0, 12)}…</button></td><td>{x.source} · {x.sourceState}</td><td>{x.semantics}</td><td>{x.metrics.cpuMilliseconds ?? "—"}</td><td>{x.metrics.durationMilliseconds ?? "—"}</td><td>{x.metrics.executions ?? "—"}</td><td>{x.metrics.logicalReads ?? "—"} / {x.metrics.writes ?? "—"} / {x.metrics.rows ?? "—"}</td><td>{x.coverage} · {x.fresh ? "fresh at snapshot" : "stale"}{x.truncated ? " · truncated" : ""}</td></tr>)}
