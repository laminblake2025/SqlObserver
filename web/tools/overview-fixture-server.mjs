...
createServer(async(req,res)=>{try{
 ...
 if(url.pathname==='/api/v1/overview'){
  ...
  const evidence=selected.map((t,index)=>{
   ...
   const metric=(key,unit,base)=>({targetId:t.instanceId,label:t.displayName,metric:key,unit,state:gap?'partial':'observed',dimension:null,points:gap?[]:Array.from({length:48},(_,j)=>({timeUtc:new Date(from+(to-from)*j/48).toISOString(),value:Math.max(0,base*(1+.25*Math.sin(j/5+i))+(i===1&&j>27&&j<34?base*.7:0)),samples:10}))});
