...
async function fixture() {
  ...
  const components = products.map(([, name]) => { const version = name === "SqlObserver.Web" ? commit : "1.0.0"; return { type: "application", name, version, "bom-ref": `pkg:generic/${name.toLowerCase()}@${version}`, purl: `pkg:generic/${name.toLowerCase()}@${version}` }; });
...
test("malformed CycloneDX root, missing dependencies, and extra root are rejected", async () => {
  const f = await fixture(); try {
    ...
    for (const mutate of [value => { delete value.serialNumber; }, value => { delete value.dependencies; }, value => { value.metadata.component = { ...value.metadata.component, "bom-ref": "pkg:generic/extra@x" }; }, value => { const web = value.components.find((item) => item.name === "SqlObserver.Web"); const oldRef = web["bom-ref"]; web.version = "1.0.0"; web["bom-ref"] = "pkg:generic/sqlobserver.web@1.0.0"; web.purl = web["bom-ref"]; for (const edge of value.dependencies) { if (edge.ref === oldRef) edge.ref = web["bom-ref"]; edge.dependsOn = edge.dependsOn.map((ref) => ref === oldRef ? web["bom-ref"] : ref).sort(); } value.dependencies.sort((a, b) => a.ref.localeCompare(b.ref)); }]) {
