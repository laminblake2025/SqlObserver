...
public sealed class M7QueryPerformanceIntegrationTests
{
    ...
    [Fact] public void TopRankingUsesOneLatestObservationPerLogicalIdentity() { string path = Path.GetFullPath(Path.Combine(AppContext.BaseDirectory, "../../../../../database/migrations/0068_query_ranking_latest_observation.sql")); string migration = File.ReadAllText(path); Assert.Contains("SELECT DISTINCT ON (o.database_id,o.query_fingerprint,o.plan_fingerprint,o.source,o.semantics)", migration, StringComparison.Ordinal); Assert.Contains("o.interval_end DESC,o.observed_at DESC,r.committed_at DESC", migration, StringComparison.Ordinal); Assert.Contains("FROM latest_observations AS latest", migration, StringComparison.Ordinal); }
    [Fact] public void DatabaseCatalogUsesCurrentTargetInventoryNamesAndQueryPerformanceBounds() { string path = Path.GetFullPath(Path.Combine(AppContext.BaseDirectory, "../../../../../database/migrations/0069_query_performance_database_catalog.sql")); string migration = File.ReadAllText(path); Assert.Contains("telemetry.database_inventory_snapshot", migration, StringComparison.Ordinal); Assert.Contains("run.collector_id = 'database.inventory'", migration, StringComparison.Ordinal); Assert.Contains("snapshot.database_id > 4", migration, StringComparison.Ordinal); Assert.Contains("snapshot.state_code = 'online'", migration, StringComparison.Ordinal); Assert.Contains("snapshot.user_access = 'multi_user'", migration, StringComparison.Ordinal); Assert.Contains("control.query_performance_target_claim(p_instance_id)", migration, StringComparison.Ordinal); Assert.Contains("LIMIT 256", migration, StringComparison.Ordinal); Assert.Contains("GRANT EXECUTE ON FUNCTION control.get_query_performance_database_catalog(uuid) TO sqlobserver_server", migration, StringComparison.Ordinal); }
