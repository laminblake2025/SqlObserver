...
public sealed class M12ReportsPostgreSqlIntegrationTests(PostgreSql18Fixture fixture)
{
    ...
    public async Task ReportCreationIsIdempotentScopedAndMaterializedAtomically()
    {
        ...
        // Runtime roles read through the scoped functions; direct table access
        // must remain denied even when the caller has the matching target scope.
    ...
    public async Task AllFixedMaterializersCreateSnapshotBoundRuns()
    {
        ...
        await using NpgsqlDataSource server = database.CreateServerDataSource(); await using NpgsqlConnection connection = await server.OpenConnectionAsync();
        await using (var scope = new NpgsqlCommand("SELECT set_config('sqlobserver.target_scope',@scope,false);", connection)) { scope.Parameters.AddWithValue("scope", target.ToString("D")); await scope.ExecuteNonQueryAsync(); }
        // Inspect persisted materialization using the fixture administrator.
        // The server role intentionally has function EXECUTE, not table SELECT.
        ...
        foreach (string kind in kinds)
        {
            ...
            await using var check = new NpgsqlCommand(
                ...
                SELECT run.state,run.report_kind,
                    (SELECT count(*) FROM reporting.report_section_row AS row WHERE row.run_id=run.run_id),
                    ...
                    (SELECT count(*) FROM reporting.report_section_row AS row
                     WHERE row.run_id=run.run_id AND NOT EXISTS
                         (SELECT 1 FROM reporting.report_definition AS definition
                          WHERE definition.report_kind=run.report_kind
                            AND definition.definition_version=run.definition_version
                            AND row.section=ANY(definition.sections))),
                    run.expires_at_utc=run.snapshot_utc+interval '24 hours'
                FROM reporting.report_run AS run WHERE run.run_id=@run;
                """, inspection);
            ...
            Assert.True(result.GetBoolean(5));
        }
    }
}
