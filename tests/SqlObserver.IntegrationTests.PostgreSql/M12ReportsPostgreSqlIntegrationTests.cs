...
public sealed class M12ReportsPostgreSqlIntegrationTests(PostgreSql18Fixture fixture)
{
    ...
    public async Task ReportCreationIsIdempotentScopedAndMaterializedAtomically()
    {
        ...
        {
            await using var insert = new NpgsqlCommand("INSERT INTO control.observation_target(instance_id,instance_key,display_name) VALUES(@id,@key,'report test'),(@other,@otherkey,'other report test');", admin); insert.Parameters.AddWithValue("id", target); insert.Parameters.AddWithValue("key", $"report-{target:N}"); insert.Parameters.AddWithValue("other", otherTarget); insert.Parameters.AddWithValue("otherkey", $"report-{otherTarget:N}"); await insert.ExecuteNonQueryAsync();
        }
        ...
        await using var count = new NpgsqlCommand("SELECT count(*) FROM reporting.report_section_row WHERE run_id=@run;", connection); count.Parameters.AddWithValue("run", first); Assert.Equal(0, Convert.ToInt32(await count.ExecuteScalarAsync(), System.Globalization.CultureInfo.InvariantCulture));
        Guid otherRun = Guid.NewGuid();
        await using (NpgsqlConnection admin = await database.DataSource.OpenConnectionAsync())
        {
            await using var seed = new NpgsqlCommand("INSERT INTO reporting.report_run(run_id,target_id,target_revision,report_kind,definition_version,operation_id,actor_sid,parameter_digest,snapshot_utc,expires_at_utc,state) VALUES(@run,@target,1,'instance-health',1,@op,'S-1-5-21-1',decode(repeat('22',32),'hex'),clock_timestamp(),clock_timestamp()+interval '24 hours','finalized'); INSERT INTO reporting.report_section_row(run_id,section,ordinal,\"values\") VALUES(@run,'health',1,ARRAY['other','safe']);", admin); seed.Parameters.AddWithValue("run", otherRun); seed.Parameters.AddWithValue("target", otherTarget); seed.Parameters.AddWithValue("op", Guid.NewGuid()); await seed.ExecuteNonQueryAsync();
        }
    ...
    public async Task FencedExpiryRemovesExpiredRowsWithoutTargetScopeFiltering()
    {
        ...
        await using (NpgsqlConnection admin = await database.DataSource.OpenConnectionAsync())
        {
            await using var targetInsert = new NpgsqlCommand("INSERT INTO control.observation_target(instance_id,instance_key,display_name) VALUES(@id,@key,'expiry test');", admin); targetInsert.Parameters.AddWithValue("id", target); targetInsert.Parameters.AddWithValue("key", $"expiry-{target:N}"); await targetInsert.ExecuteNonQueryAsync();
            await using var rows = new NpgsqlCommand("INSERT INTO reporting.report_run(run_id,target_id,target_revision,report_kind,definition_version,operation_id,actor_sid,parameter_digest,snapshot_utc,expires_at_utc,state,created_at) VALUES(@expired,@target,1,'instance-health',1,@op1,'S-1-5-21-1',decode(repeat('00',32),'hex'),clock_timestamp()-interval '25 hours',clock_timestamp()-interval '1 hour','finalized',clock_timestamp()-interval '25 hours'),(@live,@target,1,'instance-health',1,@op2,'S-1-5-21-1',decode(repeat('11',32),'hex'),clock_timestamp(),clock_timestamp()+interval '24 hours','finalized',clock_timestamp());", admin); rows.Parameters.AddWithValue("expired", expired); rows.Parameters.AddWithValue("live", live); rows.Parameters.AddWithValue("target", target); rows.Parameters.AddWithValue("op1", Guid.NewGuid()); rows.Parameters.AddWithValue("op2", Guid.NewGuid()); await rows.ExecuteNonQueryAsync();
    ...
    public async Task AllFixedMaterializersCreateSnapshotBoundRuns()
    {
        ...
        await using (NpgsqlConnection admin = await database.DataSource.OpenConnectionAsync()) { await using var insert = new NpgsqlCommand("INSERT INTO control.observation_target(instance_id,instance_key,display_name) VALUES(@id,@key,'materializer test');", admin); insert.Parameters.AddWithValue("id", target); insert.Parameters.AddWithValue("key", $"materializer-{target:N}"); await insert.ExecuteNonQueryAsync(); }
        await using NpgsqlDataSource server = database.CreateServerDataSource(); await using NpgsqlConnection connection = await server.OpenConnectionAsync();
        await using (var scope = new NpgsqlCommand("SELECT set_config('sqlobserver.target_scope',@scope,false);", connection)) { scope.Parameters.AddWithValue("scope", target.ToString("D")); await scope.ExecuteNonQueryAsync(); }
        ...
        foreach (string kind in kinds)
        {
            ...
            await using var check = new NpgsqlCommand("SELECT state,report_kind,(SELECT count(*) FROM reporting.report_section_row WHERE run_id=@run AND section=ANY((SELECT sections FROM reporting.report_definition WHERE report_kind=@kind AND definition_version=1))), (SELECT count(*) FROM reporting.report_section_row WHERE run_id=@run AND EXISTS (SELECT 1 FROM unnest(\"values\") AS item WHERE item ~ '[[:cntrl:]]')) FROM reporting.report_run WHERE run_id=@run;", connection); check.Parameters.AddWithValue("run", run); check.Parameters.AddWithValue("kind", kind); await using NpgsqlDataReader result = await check.ExecuteReaderAsync(); Assert.True(await result.ReadAsync()); Assert.Equal("finalized", result.GetString(0)); Assert.Equal(kind, result.GetString(1)); Assert.True(result.GetInt64(2) <= 10000); Assert.Equal(0L, result.GetInt64(3));
        }
    }
}
