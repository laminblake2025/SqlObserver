...
dispatch. It binds the exact SBOM and 41-file input manifest to the exact
...
- Reports and exports: locally implemented as the bounded, target-scoped
  ...
  including migration `0021` and its append-only PostgreSQL 18 runtime repairs
  in migrations `0022` and `0023`. External report-volume, browser/accessibility, and
