...
This directory is the operator-runbook index. Milestone implementation records
describe locally verified behavior, but production procedures must still be
written, exercised, and versioned during M12 release hardening. Empty or
speculative instructions must not be presented as safe operational guidance.
...
- M11 MCP catalog/digest verification, Windows-authentication failure, RBAC
  denial, terminal-audit failure, concurrency saturation, stdio endpoint
  validation, access revocation, and bounded diagnostic troubleshooting;
...
- alert delivery and report generation;
