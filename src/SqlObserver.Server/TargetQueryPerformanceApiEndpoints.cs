...
public static class TargetQueryPerformanceApiEndpoints
{
    ...
    private static async Task<IResult> TopAsync(HttpContext h, Guid instanceId, IQueryPerformanceApiQueryService s, WindowsGroupRoleResolver r, string? metric=null, int limit=25, string? fromUtc=null, string? toUtc=null, string? cursor=null, int? databaseId=null, string? source=null, CancellationToken c=default)
    {
        try
        {
            if (limit is <= 0 or > 200 || databaseId is <= 0 or > 32767) throw new ArgumentException("Page limit or database is outside its bounds.");
            RequireExplicitCursorWindow(cursor, fromUtc, toUtc);
            DateTimeOffset to = Parse(toUtc) ?? DateTimeOffset.UtcNow, from = Parse(fromUtc) ?? to.AddHours(-24);
            QueryPerformanceMetric m = ParseMetric(metric);
            QueryPerformanceSource? filterSource = ParseSourceFilter(source);
            var decoded = DecodeCursor(cursor, instanceId, m, from, to, true);
            if (decoded is not null && (decoded.FilterDatabaseId != databaseId || decoded.FilterSource != filterSource))
                throw new ArgumentException("Cursor filter binding mismatch.");
            var request = new TopQueryRequest(new MonitoredInstanceId(instanceId), from, to, m, limit, decoded, new RepositoryCallTimeout(TimeSpan.FromSeconds(5))) { DatabaseId = databaseId, Source = filterSource };
            var page = await s.GetTopAsync(r.Resolve(h.User), request, c);
            string? next = page.HasMore && page.Items.Count > 0 ? EncodeCursor(new QueryPerformanceCursorEnvelope(request.TargetId, page.Items[^1].Query.DatabaseId, from, to, m, page.SnapshotUtc, page.Items[^1].IntervalEndUtc, page.Items[^1].Query.QueryFingerprint, page.Items[^1].Value, page.Items[^1].CollectionRunId, page.Items[^1].Plan?.PlanFingerprint, page.Items[^1].ObservationKey) { FilterDatabaseId = databaseId, FilterSource = filterSource }) : null;
            return Results.Ok(new { targetId=instanceId, metric=MetricName(m), items=page.Items.Select(MapTopItem), nextCursor=next, snapshotUtc=page.SnapshotUtc, fromUtc=from, toUtc=to });
        }
    ...
    private static QueryPerformanceSource? ParseSourceFilter(string? value) => value switch { null => null, "query_store" => QueryPerformanceSource.QueryStore, "plan_cache" => QueryPerformanceSource.PlanCache, _ => throw new ArgumentException("Source filter is not allowlisted.") };
    ...
    private static QueryPerformanceCursorEnvelope? DecodeCursor(string? x, Guid target, QueryPerformanceMetric metric, DateTimeOffset from, DateTimeOffset to, bool requireMetricValue = false)
    {
        ...
        try
        {
            ...
            var allowed = new HashSet<string>(StringComparer.Ordinal) { "targetId","databaseId","fromUtc","toUtc","metric","metricValue","planFingerprint","observationKey","collectionRunId","snapshotUtc","intervalEndUtc","queryFingerprint","filterDatabaseId","filterSource" };
            ...
            {
                FilterDatabaseId = a.TryGetProperty("filterDatabaseId", out var filterDatabase) && filterDatabase.ValueKind != JsonValueKind.Null ? filterDatabase.GetInt32() : null,
                FilterSource = a.TryGetProperty("filterSource", out var filterSource) && filterSource.ValueKind != JsonValueKind.Null ? ParseSourceFilter(filterSource.GetString()) : null
            };
            if (c.FilterDatabaseId is <= 0 or > 32767) throw new ArgumentException("Cursor database filter is outside its bounds.");
