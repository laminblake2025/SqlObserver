...
public sealed partial class PostgreSqlAnalyticsRepositoryPort : IAnalyticsRepositoryPort, IAnalyticsSurfaceRepositoryPort, IRetentionRepositoryPort, IRetentionPolicyRepositoryPort, IAnalyticsDerivationStore, IMetricSeriesProjectionRepositoryPort, IStorageForecastProjectionRepositoryPort, IDiagnosticEventProjectionRepositoryPort, IIncidentEvidenceProjectionRepositoryPort, IIncidentListProjectionRepositoryPort
{
    ...
    public async ValueTask<AnalyticsSurfacePage> ReadSurfaceAsync(Guid targetId, string surface, DateTimeOffset fromUtc, DateTimeOffset toUtc, int limit, string? cursor, CancellationToken cancellationToken)
    {
        ...
        _ = surface switch { "host/status" or "host/metrics" or "replication/status" or "replication/evidence" or "diagnostics/search" or "jobs" or "backfill" or "incidents" or "evidence-packets" => surface, _ => throw new ArgumentException("Unknown analytics surface.", nameof(surface)) };
        ...
        string sql = surface switch
        {
            ...
            "backfill" => "SELECT job_id,job_kind,status,requested_at,started_at,completed_at,attempt,target_revision FROM reporting.list_m10_backfill_jobs(@instance_id,@target_revision,@limit,@cursor_at,@cursor_id,@snapshot_utc);",
        ...
        while (await reader.ReadAsync(cancellationToken).ConfigureAwait(false))
        {
            object item = surface switch
            {
                ...
                "host/metrics" => new { observedAtUtc = ReadUtc(reader, 0), runId = reader.GetGuid(1), metricKey = reader.GetString(2), value = reader.GetDouble(3), dimensions = ParseDimensions(reader.GetFieldValue<string>(4)), targetRevision = reader.GetInt64(5) },
                "replication/status" or "replication/evidence" => new { observedAtUtc = ReadUtc(reader, 0), runId = reader.GetGuid(1), role = reader.GetString(2), synchronizationState = reader.GetString(3), sendQueueBytes = (long?)null, redoQueueBytes = (long?)null, pendingCommands = reader.IsDBNull(10) ? (long?)null : reader.GetInt64(10), latencySeconds = reader.IsDBNull(11) ? (double?)null : reader.GetDouble(11), visibilityScope = reader.GetInt16(6), coverage = reader.GetInt16(6) switch { 1 => "complete", 2 => "local_summary", _ => "visibility_gap" }, visibilityGap = reader.GetInt16(6) == 3 ? new { kind = "visibility_gap", reason = "distribution_database_unbound", identity = Convert.ToHexString(reader.GetFieldValue<byte[]>(9)).ToLowerInvariant() } : null, stateAvailable = reader.GetBoolean(7), targetRevision = reader.GetInt64(8) },
            ...
            DateTimeOffset at = surface switch { "host/status" => ReadUtc(reader, 6), "jobs" or "backfill" => ReadUtc(reader, 3), "incidents" => ReadUtc(reader, 1), _ => ReadUtc(reader, 0) };
            Guid tieId = reader.GetGuid(surface switch { "host/status" => 0, "host/metrics" or "replication/status" or "replication/evidence" => 1, "diagnostics/search" => 1, "jobs" or "backfill" => 0, "incidents" => 0, _ => 1 });
    ...
    private static void ValidateQuery(AnalyticsQueryRequest request) { ArgumentNullException.ThrowIfNull(request); if (request.TargetId is null || request.MetricKey is null || request.FromUtc.Offset != TimeSpan.Zero || request.ToUtc.Offset != TimeSpan.Zero || (request.SnapshotUtc is { } snapshot && snapshot.Offset != TimeSpan.Zero) || request.ToUtc <= request.FromUtc || request.ToUtc - request.FromUtc > TimeSpan.FromDays(90) || request.Limit is < 1 or > 100_000 || request.DimensionsSha256 is { } hash && (hash.Length != 64 || !hash.All(Uri.IsHexDigit)) || request.Dimensions is not null && !string.Equals(CanonicalDimensions.Sha256(request.Dimensions), request.DimensionsSha256 ?? CanonicalDimensions.Sha256(null), StringComparison.OrdinalIgnoreCase)) throw new ArgumentException("Analytics query is outside its bounded contract.", nameof(request)); }
    ...
    private void ValidateHostBindingMutation(HostBindingMutationRequest request, out Guid hostId, out string hostName, out string fingerprint, out long bindingRevision, out long profileRevision)
    {
        ...
        string[] allowedProfileFields = ["osFamily", "osVersion", "cpuCount", "memoryBytes", "capabilityState", "capabilities"];
        ...
        if (profile.TryGetProperty("capabilities", out JsonElement capabilities) &&
            (capabilities.ValueKind != JsonValueKind.Number || !capabilities.TryGetInt32(out int flags) || flags < 0 || (flags & ~15) != 0))
            throw new ArgumentException("Host metric capabilities are invalid.", nameof(request));
