...
public sealed class PostgreSqlQueryTextReadRepositoryPort(NpgsqlDataSource dataSource) : IQueryTextReadRepositoryPort
{
        ...
        QueryTextReadRequest request, CancellationToken cancellationToken)
    {
        ...
        await using var command = new NpgsqlCommand(
            "SELECT * FROM control.get_query_text_payload(@target,@run,@database,@query);", connection)
        {
        ...
        if (await reader.ReadAsync(timeout.Token))
            throw new InvalidDataException("Query text lookup returned more than one payload.");
        return payload;
    }

    public async ValueTask AuditAsync(
        QueryTextReadRequest request, string actorSid, string outcome, CancellationToken cancellationToken)
    {
        using CancellationTokenSource timeout = PostgreSqlRuntimeSupport.CreateTimeoutScope(request.Timeout, cancellationToken);
        await using NpgsqlConnection connection = await dataSource.OpenConnectionAsync(timeout.Token);
        await using var command = new NpgsqlCommand(
            "SELECT control.audit_query_text_access(@target,@run,@database,@query,@actor,@outcome);", connection)
        {
            CommandTimeout = PostgreSqlRuntimeSupport.GetCommandTimeoutSeconds(request.Timeout)
        };
        AddIdentity(command, request);
    ...
    private static void AddIdentity(NpgsqlCommand command, QueryTextReadRequest request)
    {
        command.Parameters.AddWithValue("target", request.TargetId.Value);
        command.Parameters.AddWithValue("run", request.CollectionRunId);
        command.Parameters.AddWithValue("database", request.Query.DatabaseId);
        command.Parameters.AddWithValue("query", Convert.FromHexString(request.Query.QueryFingerprint));
    }
}
