        ...
        DateTimeOffset fromUtc, DateTimeOffset toUtc, DateTimeOffset cutoffUtc, CancellationToken cancellationToken)
    {
        ...
        await using (var reader = await command.ExecuteReaderAsync(cancellationToken))
        {
            ...
            while (await reader.ReadAsync(cancellationToken))
            {
                ...
                if (key is not ("engine.user_connections" or "engine.batch_requests_per_second" or "engine.process_physical_memory_bytes" or "engine.os_available_memory_bytes" or "engine.memory_grants_pending" or "engine.scheduler_runnable_tasks" or "engine.sql_scheduler_cpu_percent")) throw new InvalidDataException();
