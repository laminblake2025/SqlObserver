...
public sealed class QueryTextReadService(
    IQueryTextReadRepositoryPort repository,
    IQuerySensitiveContentProtector protector) : IQueryTextReadService
{
    ...
    public async ValueTask<QueryTextReadResult> ReadAsync(
        AuthorizationContext authorization, QueryTextReadRequest request, CancellationToken cancellationToken)
    {
        ArgumentNullException.ThrowIfNull(authorization);
        ArgumentNullException.ThrowIfNull(request);
        if (request.CollectionRunId == Guid.Empty) throw new ArgumentException("A collection run is required.", nameof(request));
        ...
        try
        {
            if (payload is not null && protector.IsAvailable && payload.Kind == SensitivePayloadKind.QueryText)
            {
                try
                {
                    plaintext = await protector.UnprotectAsync(request.TargetId.Value, payload, cancellationToken);
                    if (plaintext.Length <= 16 * 1024)
                        text = new UTF8Encoding(false, true).GetString(plaintext);
                }
            ...
            await repository.AuditAsync(request, authorization.ActorSid.Value,
                text is null ? "unavailable" : "opened", cancellationToken);
            return text is null ? new("unavailable", null) : new("available", text);
        }
        finally
        {
