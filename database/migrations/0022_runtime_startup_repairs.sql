...
-- in one transaction. This append-only repair follows live PostgreSQL 18
-- startup verification of the Collector runtime.
...
-- PostgreSQL requires UPDATE in addition to SELECT for SELECT ... FOR UPDATE.
-- The dedicated NOLOGIN/BYPASSRLS role has no members, so this grant remains
-- confined to its one SECURITY DEFINER report-expiry entry point.
GRANT SELECT, UPDATE ON TABLE control.worker_lease
    TO sqlobserver_report_expirer;

-- The original set-operation attempted to order by operation_id before that
-- identity existed in the UNION output. The outer query already computes and
-- orders the canonical operation identity, so the invalid inner order is
-- removed without collapsing equal-time source samples.
CREATE OR REPLACE FUNCTION alerting.reconcile_due_evidence_internal(p_max_results integer)
...
BEGIN
  ...
  FROM (
    ...
    FROM alerting.rule r JOIN LATERAL (
      ...
      -- Equal-time sources remain distinct through sample/run/reason; the outer
      -- query orders the computed canonical operation identity.
      ) e ON true WHERE r.enabled AND (current_setting('sqlobserver.target_scope',true) IS NULL OR r.instance_id::text=current_setting('sqlobserver.target_scope',true)) AND (r.kind=1 OR EXISTS (SELECT 1 FROM reporting.collector_health_projection hp WHERE hp.instance_id=r.instance_id AND hp.collector_id='engine.core'))
  ) q ORDER BY q.observed_at ASC,q.sample_id ASC,q.run_id ASC NULLS LAST,q.operation_id ASC LIMIT p_max_results ON CONFLICT ON CONSTRAINT evaluation_queue_pkey DO NOTHING;
...
ALTER FUNCTION alerting.reconcile_due_evidence_internal(integer)
    OWNER TO sqlobserver_migrator;
REVOKE ALL ON FUNCTION alerting.reconcile_due_evidence_internal(integer)
    FROM PUBLIC, sqlobserver_server, sqlobserver_collector, sqlobserver_auditor;

-- Fail the migration itself if either live-startup repair is ineffective.
-- The nested exception block rolls back any queue reconciliation performed by
-- the execution probe while preserving a successful semantic validation.
DO $m22_verify$
BEGIN
    IF NOT has_table_privilege(
        'sqlobserver_report_expirer',
        'control.worker_lease',
        'SELECT'
        ...
        RAISE EXCEPTION 'report expiry worker-lease privilege repair failed'
            USING ERRCODE = '42501';
    END IF;

    BEGIN
        PERFORM *
        FROM alerting.reconcile_due_evidence_internal(1);

        RAISE EXCEPTION 'm22_runtime_probe_rollback'
            USING ERRCODE = 'PZ001';
    EXCEPTION
...
COMMENT ON FUNCTION alerting.reconcile_due_evidence_internal(integer) IS
    'Internal bounded alert evidence reconciliation; runtime-validated by migration 0022.';
