...
-- fenced lifecycle and accepts historical 8/9 metric runs alongside the new 11
...
BEGIN
 ...
 SET asset_bundle_sha256=decode('0fb5fc1ccb326611a60a800343791542010ab14cd508f18fff88efafd9bd8b3e','hex')
 WHERE collector_id IN ('engine.core','database.inventory','database.files') AND collector_version=1
   AND asset_bundle_sha256=decode('34214cef39c56f1d984bee1da82fd40ac410552eca04f6bd64420b001bd3114c','hex');
 ...
 WITH runs AS (
  ...
  WHERE s.instance_id=p_instance_id AND r.target_revision=p_revision AND r.collector_id='engine.core'
   ...
   AND s.metric_key IN ('engine.user_connections','engine.batch_requests_total',
    ...
    'engine.os_available_memory_bytes','engine.scheduler_runnable_tasks')
  GROUP BY r.run_id
 ), paired AS (
