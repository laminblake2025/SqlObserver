-- Preserve the fenced M4 commit contract while recording target clock skew as
-- rejected output and a visibility gap. Historical metric sets remain accepted.
-- Keep the full function here rather than rewriting pg_get_functiondef text.
SET LOCAL ROLE sqlobserver_migrator;
SET LOCAL lock_timeout = '5s';
SET LOCAL statement_timeout = '5min';
SET LOCAL TIME ZONE 'UTC';

-- The old validated constraint is a strict subset of this new allowlist. Add
-- the replacement without scanning a potentially large historical gap table;
-- NOT VALID still enforces the allowlist for every new row.
...
ALTER TABLE telemetry.visibility_gap ADD CONSTRAINT ck_visibility_gap_reason CHECK
 ...
 'blocking_graph_limit','visibility_incomplete','target_clock_skew')) NOT VALID;
...
BEGIN
    ...
    IF NOT FOUND
       ...
       (
           ...
           AND
           (
               metric_count NOT IN (8, 9, 11, 12, 14)
               OR ARRAY
                  (
                      ...
                      'engine.page_life_expectancy_seconds',
                      'engine.process_physical_memory_bytes',
                      'engine.scheduler_cpu_milliseconds_total',
                      'engine.scheduler_runnable_tasks',
                      'engine.sql_compilations_total',
                      'engine.sql_recompilations_total',
                      'engine.start_time_key',
                      'engine.target_memory_bytes',
                      'engine.user_connections',
                      'engine.visible_scheduler_count'
                  ]::text[]
                      ELSE NULL::text[]
                  END
           )
    ...
    -- Null or non-finite timestamps remain malformed input. Target clocks outside
    -- the five-minute window are a visibility gap, not an uncommittable run.
    IF EXISTS (SELECT 1 FROM unnest(p_metric_observed_ats) AS observation(observed_at)
               WHERE observation.observed_at IS NULL OR NOT isfinite(observation.observed_at))
       OR EXISTS (SELECT 1 FROM unnest(p_database_observed_ats) AS observation(observed_at)
                  WHERE observation.observed_at IS NULL OR NOT isfinite(observation.observed_at))
       OR EXISTS (SELECT 1 FROM unnest(p_file_observed_ats) AS observation(observed_at)
                  WHERE observation.observed_at IS NULL OR NOT isfinite(observation.observed_at))
    THEN
        RAISE EXCEPTION 'collector observation timestamp is invalid' USING ERRCODE = '22023';
    END IF;
    ...
    IF EXISTS
       (
           ...
           WHERE observation.observed_at < begin_started_at - interval '5 minutes'
       ...
       (
        ...
        -- The completion digest above commits to the original submitted rows.
        -- Suppressing all three payload families preserves replay identity while
        -- preventing any mis-timed sample from entering history or partitions.
...
-- The directional-file-stall entry point handles database.inventory and
-- database.files; give it the same skew treatment as engine.core.
CREATE OR REPLACE FUNCTION control.commit_collection_run_v2
(
    p_run_id uuid,
    p_instance_id uuid,
    p_target_revision bigint,
