-- Raw metric sample identities are UUIDs; compare like types while preserving
-- exact claim, source-evidence, revision and replay validation.
...
-- Disambiguate procedural values from JSON expansion aliases.
...
BEGIN
  ...
  FOR v_decision IN SELECT value FROM jsonb_array_elements(p_decisions) LOOP
    ...
    IF jsonb_typeof(v_decision->'State') <> 'object' OR (SELECT count(*) FROM jsonb_object_keys(v_decision->'State')) <> 20 OR EXISTS (SELECT 1 FROM jsonb_object_keys(v_decision->'State') k WHERE k NOT IN ('RuleId','TargetId','State','ConsecutiveMatches','FirstMatchUtc','LastObservedUtc','FiredUtc','AcknowledgedUtc','ResolvedUtc','EpisodeStartedUtc','AlertId','EpisodeId','LastOperationId','EvidenceDigest','Reason','LastReason','DeliverySuppressed','AcknowledgedBy','Revision','LastValue')) THEN RAISE EXCEPTION 'alert decision state snapshot is malformed' USING ERRCODE='22023'; END IF;
    ...
    IF FOUND THEN
      IF history_digest IS DISTINCT FROM evidence OR replay_result_digest IS DISTINCT FROM sha256(convert_to(alerting.canonical_decision_snapshot(v_decision)::text,'UTF8')) OR replay_target_binding IS DISTINCT FROM target OR replay_rule_revision IS DISTINCT FROM (SELECT r.revision FROM alerting.rule r WHERE r.instance_id=target AND r.rule_id=rule) OR replay_rule_binding IS DISTINCT FROM (SELECT to_jsonb(r) FROM alerting.rule r WHERE r.instance_id=target AND r.rule_id=rule) THEN RAISE EXCEPTION 'divergent alert replay' USING ERRCODE='40001'; END IF;
      CONTINUE;
    END IF;
    ...
    IF next_state IS DISTINCT FROM expected_state OR consecutive IS DISTINCT FROM expected_consecutive OR NULLIF(v_decision->'State'->>'FirstMatchUtc','')::timestamptz IS DISTINCT FROM expected_first THEN RAISE EXCEPTION 'alert decision state does not match repository evaluation' USING ERRCODE='22023'; END IF;
