-- M11: immutable forecast continuation with a complete (horizon_start, id)
-- tie tuple. The limit is intentionally bounded to the application page plus
-- one lookahead row.
...
REVOKE ALL ON FUNCTION reporting.get_m10_forecast_scoped(uuid,bigint,text,jsonb,interval,timestamptz,integer,timestamptz,uuid)
    FROM PUBLIC,sqlobserver_collector,sqlobserver_auditor;
GRANT EXECUTE ON FUNCTION reporting.get_m10_forecast_scoped(uuid,bigint,text,jsonb,interval,timestamptz,integer,timestamptz,uuid)
