-- Use the observation target key directly where present. Historical NULL rows
-- keep their exact query-ownership fallback until the bounded backfill finishes.
-- Preserve latest-observation, cursor and snapshot semantics during the rollout.
SET LOCAL ROLE sqlobserver_migrator;
SET LOCAL lock_timeout = '5s';
SET LOCAL statement_timeout = '5min';

DO $migration$
DECLARE role_suffix text; predicate text;
BEGIN
  FOREACH role_suffix IN ARRAY ARRAY['collector','server','migrator'] LOOP
    predicate := $predicate$
      (SELECT control.query_performance_target_claim(
      ...
      AND (instance_id = nullif(current_setting('sqlobserver.target_scope',true),'')::uuid
        OR (instance_id IS NULL AND EXISTS (
          SELECT 1 FROM events.query_performance_query AS q
          WHERE q.collection_run_id=query_performance_observation.collection_run_id
            ...
            AND q.instance_id=nullif(current_setting('sqlobserver.target_scope',true),'')::uuid)))
      $predicate$;
    EXECUTE format('ALTER POLICY %I ON events.query_performance_observation USING (%s)',
      'query_performance_observation_' || role_suffix,predicate);
