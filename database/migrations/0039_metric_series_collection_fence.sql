-- Restore collected-at snapshot fencing and require complete aggregate inputs.
...
LANGUAGE sql STABLE SECURITY DEFINER SET search_path=pg_catalog,reporting,telemetry,control SET TimeZone='UTC' AS $$
 WITH source AS (
  ...
  WHERE h.instance_id=p_instance_id AND h.target_revision=p_target_revision AND h.metric_key=p_metric_key
    AND h.collected_at<=p_snapshot_utc AND h.observed_at>=p_from_utc AND h.observed_at<p_to_utc AND h.observed_at<=p_snapshot_utc
  UNION ALL
  ...
  WHERE r.instance_id=p_instance_id AND r.target_revision=p_target_revision
    ...
    AND r.collected_at<=p_snapshot_utc AND r.observed_at>=p_from_utc AND r.observed_at<p_to_utc AND r.observed_at<=p_snapshot_utc
  GROUP BY r.observed_at,r.run_id
  HAVING bool_and(r.visibility_scope=1 AND CASE p_metric_key WHEN 'replication.latency_seconds' THEN r.latency_seconds IS NOT NULL ELSE r.pending_commands IS NOT NULL END)
