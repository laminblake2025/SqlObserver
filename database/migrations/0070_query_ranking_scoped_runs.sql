-- Scope ranking to the target's committed runs once before scanning the
-- requested observation window. The observation FORCE RLS policy continues to
-- enforce the exact query-ownership tuple; the explicit query join used by
-- earlier ranking functions repeated that ownership lookup for every row.
-- Keep the latest-per-query/plan/source/semantics and cursor ordering stable.
