-- Preserve source-local SQL Agent execution starts without inventing UTC.
-- Metadata is captured per run; immutable event identity and first observation remain unchanged.
...
ALTER TABLE telemetry.sql_agent_failure_occurrence
    ADD COLUMN source_local_start timestamp without time zone,
    ADD CONSTRAINT ck_agent_source_local_start CHECK (
        source_local_start IS NULL OR (
            isfinite(source_local_start)
            AND source_local_start >= timestamp '0001-01-01 00:00:00'
            AND source_local_start < timestamp '10000-01-01 00:00:00'
            AND date_trunc('second',source_local_start)=source_local_start
        )
    );
...
BEGIN
 ...
 IF NOT is_failure AND p_collector_id='sql-agent.failures' THEN
   FOR source_row IN SELECT value FROM jsonb_array_elements(COALESCE(p_payload->'items','[]'::jsonb)) LOOP
     IF source_row ? 'sourceLocalStart' AND source_row->'sourceLocalStart' <> 'null'::jsonb THEN
       IF jsonb_typeof(source_row->'sourceLocalStart') <> 'string'
          OR source_row->>'sourceLocalStart' !~ '^[0-9]{4}-[0-9]{2}-[0-9]{2}T[0-9]{2}:[0-9]{2}:[0-9]{2}$' THEN
         RAISE EXCEPTION 'Agent source-local start must be an offset-free whole-second timestamp' USING ERRCODE='22023';
       END IF;
       BEGIN
         parsed_source_start := (source_row->>'sourceLocalStart')::timestamp;
       EXCEPTION WHEN invalid_datetime_format OR datetime_field_overflow THEN
         RAISE EXCEPTION 'Agent source-local start calendar value is invalid' USING ERRCODE='22023';
       END;
       IF NOT isfinite(parsed_source_start)
          OR parsed_source_start < timestamp '0001-01-01 00:00:00'
          OR parsed_source_start >= timestamp '10000-01-01 00:00:00'
          OR to_char(parsed_source_start,'YYYY-MM-DD"T"HH24:MI:SS') <> source_row->>'sourceLocalStart' THEN
         RAISE EXCEPTION 'Agent source-local start calendar value is invalid' USING ERRCODE='22023';
       END IF;
     END IF;
   END LOOP;
 ...
 SELECT p_instance_id,p_target_revision,p_run_id,x."jobId",x."historyInstanceId",x."stepId",x."runStatus",x."failureKind",x."messageId",x.severity,x."retryAttempt",x."durationSeconds",x."firstObservedAtUtc",x."firstObservedAtUtc",decode(x."failureFingerprint",'hex') FROM jsonb_to_recordset(COALESCE(p_payload->'items','[]'::jsonb)) AS x("jobId" uuid,"historyInstanceId" bigint,"stepId" integer,"runStatus" integer,"failureKind" smallint,"messageId" integer,severity integer,"retryAttempt" integer,"durationSeconds" integer,"firstObservedAtUtc" timestamptz,"failureFingerprint" text) ON CONFLICT(instance_id,target_revision,failure_fingerprint) DO NOTHING;
   ...
   WITH new_occurrences AS (
     INSERT INTO telemetry.sql_agent_failure_occurrence(instance_id,target_revision,run_id,collected_at,failure_fingerprint,source_local_start)
     SELECT p_instance_id,p_target_revision,p_run_id,(p_payload->>'observedAtUtc')::timestamptz,
            decode(x."failureFingerprint",'hex'),x."sourceLocalStart"
     FROM jsonb_to_recordset(COALESCE(p_payload->'items','[]'::jsonb)) AS x("failureFingerprint" text,"sourceLocalStart" timestamp)
     ON CONFLICT DO NOTHING RETURNING source_local_start
   ) SELECT count(*)::integer,count(source_local_start)::integer
     INTO occurrence_inserted,known_source_inserted FROM new_occurrences;
   ...
   inserted:=occurrence_inserted; duplicate_count:=greatest(p_output_item_count-occurrence_inserted,0); bytes:=inserted*160+known_source_inserted*8;
...
-- Keep the original result shape callable by older readers.
CREATE FUNCTION reporting.list_sql_agent_failures_v2(
    p_instance_id uuid,p_snapshot_run_id uuid,p_target_revision bigint,
    p_from timestamptz,p_to timestamptz,p_after_detected timestamptz,
    p_after_fingerprint bytea,p_limit integer)
RETURNS TABLE(job_id uuid,history_instance_id bigint,step_id integer,run_status integer,
    failure_kind smallint,message_id integer,severity integer,retry_attempt integer,
    duration_seconds integer,detected_at timestamptz,failure_fingerprint bytea,source_local_start timestamp)
LANGUAGE plpgsql STABLE SECURITY DEFINER
SET search_path=pg_catalog,reporting,telemetry,control SET TimeZone='UTC' AS $read$
BEGIN
    IF current_setting('sqlobserver.target_scope',true) IS DISTINCT FROM p_instance_id::text THEN
        RAISE EXCEPTION 'Agent history target scope rejected' USING ERRCODE='42501';
    END IF;
    IF p_instance_id IS NULL OR p_snapshot_run_id IS NULL OR p_target_revision IS NULL OR p_target_revision<1
       OR p_from IS NULL OR p_to IS NULL OR NOT isfinite(p_from) OR NOT isfinite(p_to)
       OR p_to<=p_from OR p_to-p_from>interval '7 days'
       OR p_limit IS NULL OR p_limit NOT BETWEEN 1 AND 201
       OR (p_after_detected IS NULL)<>(p_after_fingerprint IS NULL)
       OR p_after_detected IS NOT NULL AND
          (NOT isfinite(p_after_detected) OR p_after_detected<p_from OR p_after_detected>p_to
           OR octet_length(p_after_fingerprint)<>32) THEN
        RAISE EXCEPTION 'Agent history projection bounds rejected' USING ERRCODE='22023';
    END IF;
    RETURN QUERY
    SELECT failure.job_id,failure.history_instance_id,failure.step_id,failure.run_status,
           failure.failure_kind,failure.message_id,failure.severity,failure.retry_attempt,
           failure.duration_seconds,failure.first_observed_at_utc,failure.failure_fingerprint,occurrence.source_local_start
    FROM telemetry.sql_agent_failure_occurrence occurrence
    JOIN telemetry.sql_agent_failure failure
      ON failure.instance_id=occurrence.instance_id AND failure.target_revision=occurrence.target_revision
     AND failure.failure_fingerprint=occurrence.failure_fingerprint
    WHERE occurrence.instance_id=p_instance_id AND occurrence.run_id=p_snapshot_run_id
      AND occurrence.target_revision=p_target_revision
      AND failure.first_observed_at_utc BETWEEN p_from AND p_to
      AND (p_after_detected IS NULL OR failure.first_observed_at_utc<p_after_detected
           OR failure.first_observed_at_utc=p_after_detected AND failure.failure_fingerprint>p_after_fingerprint)
    ORDER BY failure.first_observed_at_utc DESC,failure.failure_fingerprint
    LIMIT p_limit;
END
$read$;
REVOKE ALL ON FUNCTION reporting.list_sql_agent_failures_v2(uuid,uuid,bigint,timestamptz,timestamptz,timestamptz,bytea,integer)
    FROM PUBLIC,sqlobserver_collector,sqlobserver_auditor;
GRANT EXECUTE ON FUNCTION reporting.list_sql_agent_failures_v2(uuid,uuid,bigint,timestamptz,timestamptz,timestamptz,bytea,integer)
    TO sqlobserver_server;

-- The four operational collectors share one pinned asset bundle.
...
BEGIN
    ...
    SET asset_bundle_sha256=decode('8fa22b58b193640b94e1290fc48820f3d68afdfda9076809f4e9b4fc3b2fa593','hex')
    WHERE collector_id IN ('backups.status','sql-agent.failures','tempdb.health','availability-groups.health')
      AND collector_version=1
      AND asset_bundle_sha256=decode('5cead2f81a535b7726c5eb6c6b4abac35cbc8cd85301b60e1867ba694e4a3e8e','hex');
